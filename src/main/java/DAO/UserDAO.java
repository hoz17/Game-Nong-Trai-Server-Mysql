...
public class UserDAO extends DAO {
    ...
    public User verifyUser(User user) throws SQLException {
        try {
            PreparedStatement preparedStatement = conn.prepareStatement("SELECT * FROM player WHERE Username = ? AND Password = ?;");
    ...
    public void addUser(User user) {
        try {
            ...
            preparedStatement.setString(4, user.getPlayerName());
            preparedStatement.setInt(5, 1000);
            preparedStatement.setInt(6, user.getGenderSkin());
            preparedStatement.setInt(7, user.getPetID());
...
//                preparedStatement.setInt(4, null);
    ...
    public String getLeaderBoard() {
        String output = "";
        try {
            PreparedStatement preparedStatement = conn.prepareStatement("SELECT Player_name, Money from player ORDER BY Money DESC LIMIT 10;");
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
